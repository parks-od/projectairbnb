$(function () {
    ...
    function lodgingIdSearch(id) {
        $.get("/api/room/roomPage/" + id, function (response) {
            ...
            console.dir(roomView.data);
            console.log(roomView.data.lodgingPicturesApiResponse);
            ...
            let isresvStart11 = roomView.data.lodgingIsresvApiResponse.isresvStart
            let isresvEnd22 = roomView.data.lodgingIsresvApiResponse.isresvEnd
            let isresvStart = isresvStart11.substring(0,10)
            let isresvEnd = isresvEnd22.substring(0,10)
            ...
            $('#ckeckOutDate').text(year+'. '+month+'. '+(day+1)+'.')
    ...
    function change(){
        ...
        let priceNN = Number($('#nomalpricepic').text().replace(",",""))
        let priceNY = priceNN*pep
        let price = priceNY.toLocaleString('ko-KR')
        $('#priceNormal1').text(price)
        $('#sideBarPrice').text(price)
        $('.pricepep').text(price)
        let bak = Number($('#sidebak').text())
    ...
    reviewSearch(id);
    function reviewSearch(id) {
        $.get("/api/review?id=" + id, function (response) {
            console.dir(response);
            reviewMain.list = response.data;
    ...
    $('#datepicker5').on('change', function (){
        ...
        for(let i = 0; i<btDay; i++){
            $('#_18x0pkv1').append(`<div class="_1fpuhdl _1fpuhdl123"><span class="_18x3iiu"><div>${start1}. ${start2}. ${start3+i}.</div></span><span class="_1k4xcdh">₩<span class="pricepep">${price}</span></span></div>`)
        }
    ...
    // let total2= Number($('#totalPrice').text().replace(",",""))
    // let total3= Number($('#sideBarPriceTotal2').text().replace(",",""))
    ...
    reviewPopupSearch(id);
    function reviewPopupSearch(id) {
        $.get("/api/review?id=" + id, function (response) {
            console.dir(response);
            reivewPopup.list = response.data;
